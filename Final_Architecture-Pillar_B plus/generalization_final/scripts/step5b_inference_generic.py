...
if __name__ == '__main__':
    # Wrap with CodeCarbon when run standalone. step5b is also imported as a
    # helper module by step5d; when imported, this __main__ block doesn't run
    # so no tracker is created.
    try:
        from codecarbon import EmissionsTracker
        _here = os.path.dirname(os.path.abspath(__file__))
        _emissions_dir = os.path.join(os.path.dirname(_here), 'outputs', 'emissions')
        os.makedirs(_emissions_dir, exist_ok=True)
        _tracker = EmissionsTracker(project_name='step5b_inference_generic',
