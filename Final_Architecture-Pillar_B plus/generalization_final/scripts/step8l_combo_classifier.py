...
class ComboMLP(nn.Module):
    """COMBO MLP with optional Gombert-style multi-task head.

    When `use_grade_reg=True`, a shared 2-layer backbone feeds two heads:
      * cls_head -> 3-class softmax (existing)
      * grade_head -> 1-dim continuous grade regression (new)
    Total loss during training = CE + grade_loss_weight * SmoothL1.

    forward() always returns a tuple (cls_logits, grade_or_None). Callers that
    only care about classification can take [0].
    """

    def __init__(self, in_dim, hidden=(256, 128), num_classes=NUM_CLASSES,
                 dropout=0.2, use_grade_reg=False):
        super().__init__()
        layers, prev = [], in_dim
        for h in hidden:
        ...
        self.grade_head = nn.Linear(prev, 1) if use_grade_reg else None

    def forward(self, x):
        h = self.backbone(x)
        cls_logits = self.cls_head(h)
        grade = (self.grade_head(h).squeeze(-1)
                 if self.grade_head is not None else None)
        return cls_logits, grade
...
GRADE_TARGET = {0: 3.0, 1: 7.0, 2: 11.0}   # mid-range US grade per class
...
def fit_one(X_train, y_train, X_val, y_val, *, epochs=120, patience=15,
            ...
            use_grade_reg=False, grade_loss_weight=0.3):
    ...
    grade_t = None
    if use_grade_reg:
        grades = np.array([GRADE_TARGET[int(c)] for c in y_train], dtype=np.float32)
        grade_t = torch.tensor(grades, dtype=torch.float32, device=device)
    model = ComboMLP(X_train.shape[1], hidden=hidden, dropout=dropout,
                     use_grade_reg=use_grade_reg).to(device)
    ...
    for epoch in range(epochs):
        ...
        for start in range(0, n, batch):
            ...
            if len(b) == 0:
                continue
            cls_logits, grade_pred = model(Xt[b])
            loss = F.cross_entropy(cls_logits, yt[b])
            if use_grade_reg and grade_pred is not None and grade_loss_weight > 0:
                loss = loss + grade_loss_weight * F.smooth_l1_loss(grade_pred, grade_t[b])
            opt.zero_grad(); loss.backward(); opt.step()
        model.eval()
        with torch.no_grad():
            cls_logits, _ = model(Xv)
...
def main():
    ...
    ap.add_argument('--use-grade-reg', action='store_true',
                    help='Add Gombert-style multi-task continuous-grade regression head.')
    ap.add_argument('--grade-loss-weight', type=float, default=0.3,
                    help='Weight of the SmoothL1 grade loss in the combined objective.')
    args = ap.parse_args()
    ...
    try:
        ...
        for llm_tag in per_llm_list:
            ...
            for seed in args.seeds:
                t0 = time.time()
                model, best_val = fit_one(
                    X_train_s, y_train, X_val_s, y_val, seed=seed,
                    use_grade_reg=args.use_grade_reg,
                ...
                report = {
                    ...
                    'llm': llm_tag,
                    'seed': seed,
                    'use_grade_reg': bool(args.use_grade_reg),
                ...
                # Append "_gradeREG" to tag when multi-task head is on so bundles
                # don't collide with the existing single-task runs.
                grade_suffix = '_gradereg' if args.use_grade_reg else ''
                tag = f'{args.config}__{args.label_source}__' \
                      f'{llm_tag if llm_tag else "all"}__seed{seed}{grade_suffix}'
