...
For the domain-adversarial ELECTRA notebooks:
...
- Some notebooks and scripts were developed in Colab or on a remote GPU box.
  ...
  before launching.
- `DomainAdversialNN/` notebooks need a CUDA GPU for ELECTRA-large training.
  They were developed mainly for Colab/Drive-style runs, so update the input and
  output paths at the top of each notebook before rerunning.
...
### 5. Domain-Adversarial ELECTRA Experiments

Trains ELECTRA-based difficulty classifiers with Domain-Adversarial Neural Network
(DANN) training. DANN was added to improve cross-corpus generalization: the model
learns to predict `elementary`, `middle`, and `high`, while a gradient reversal
domain head discourages it from relying too heavily on source-dataset identity.

The first DANN experiments used only the original training CSV, but this was not
enough for strong OOD behavior. The later experiments found that the model needs
training data from multiple datasets with more similar formatting and label style.
...
text features, and a DANN gradient reversal head. Training runs in two phases:
first with the ELECTRA encoder frozen, then with partial unfreezing and DANN
enabled.

Other notebooks in `DomainAdversialNN/` are earlier experiments with DANN and
related data setups, including smaller datasets, synthetic data, original clean
datasets, and alternate mixed-corpus runs.

To run: open `DomainAdversialNN/Electra_Static_DANN_OSE_RACE_MultiOOD.ipynb` on
a CUDA GPU runtime, install the notebook dependencies, mount Drive if using
Colab, update the input/output paths at the top, then run all cells. The notebook
saves model checkpoints, evaluation CSV/JSON files, and confusion matrices.

Saved DANN results are in `DomainAdversialNN/results/dann_static_llmAsJudge_multi_ood`:

| Checkpoint | OOD corpus | Accuracy | Macro-F1 |
|---|---|---:|---:|
| `phase2_dann_final` | CoQA | 0.8315 | 0.8255 |
| `phase2_dann_final` | CommonLit | 0.8260 | 0.8797 |

### 6. Final Pillar B+ Architecture
...
- `synthetic-data/synthetic_{claude,deepseek,mistral}_qa.csv`: synthetic
  ...
  generalization.
- `DomainAdversialNN/results/`: saved outputs from multiple DANN experiments,
  including earlier trials with different dataset combinations and training setups.
- `DomainAdversialNN/results/dann_static_llmAsJudge_multi_ood/evaluationResults.csv`:
  main DANN result file for the selected ELECTRA + DANN multi-corpus run.
- `DomainAdversialNN/results/dann_static_llmAsJudge_multi_ood/confusion_matrices/`:
  confusion matrix images for the main DANN OOD evaluations.
- DANN notebook outputs such as `best_model.pt`, `phase2_final.pt`,
  `eval_results.json`, `eval_comparison.csv`, and `static_scaler.joblib` are
  generated by the notebook run and saved to the configured Drive/output folder.
...
Maneesha's work includes:

- `logistic-regression/`
  - Built the final Rooein-style modeling and evaluation pipeline after the
    feature-generation steps, including model comparison, per-class evaluation,
    confusion matrices, bootstrap significance testing, and organized result
    exports.
  - Compared runs across Gemma-7B, Llama-2-7B, Llama-2-13B, Mistral-7B, and
    Qwen2.5-7B and added OneStopEnglish OOD evaluation and a separate OneStopEnglish in-domain
    training notebook to compare frozen transfer against training directly on
    the target corpus.

- `DomainAdversialNN/`
  - Built the domain-adversarial ELECTRA experiments for OOD generalization.
  - Started with the original training CSV, then expanded to mixed-corpus
    training after finding that OOD performance needed datasets with more
    compatible formatting and label style.
  - Added training/evaluation data from OneStopEnglish, RACE-middle, RACE-high,
    CNN/DailyMail, CoQA, CommonLit, XSum, and WeeBit across the DANN experiments.
  - Implemented the main ELECTRA + ScalarMix + DANN setup with two-phase
    training: frozen encoder warmup followed by partial unfreezing with gradient
    reversal enabled ans saved final DANN OOD evaluations and confusion matrices.

- `llm_as_a_judge/`
  - Added additional datasets and ran the judge-labeling workflow to create more
    unified labels across corpora and used it to test whether cleaner cross-corpus labels would
    generalize better in later DANN experiments.
